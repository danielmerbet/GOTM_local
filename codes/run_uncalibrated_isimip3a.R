...
for (model in c("GSWP3-W5E5", "20CRv3-ERA5")){ #"20CRv3", "GSWP3-W5E5"
  ...
  for (c in 58:73){
    ...
    #meteorology
    clim <- paste0(wd, "ISIMIP_Local_Lakes/",phase,"/",model,"/",tolower(model),"_",escenario,"_obsclim_", tolower(lakes[c]),"_daily_",years_range[1],"_",years_range[length(years_range)],".csv")
    ...
    write.table(clim_spinup, file = paste0(wd, "uncalibrated/meteo_file.dat"),
                quote = F, row.names = F, col.names = F)
    ...
    write.table(hyp_save, file = paste0(wd, "uncalibrated/hypsograph.dat"),
                quote = F, row.names = F, col.names = F)
    
    #set gotm.yaml
    gotm_ymal <- readLines(paste0(wd, "uncalibrated/gotm.yaml"))
    ...
    writeLines(gotm_ymal, con = paste0(wd, "uncalibrated/gotm.yaml"))
    
    setwd(paste0(wd, "uncalibrated/"))
    
    system("./gotm")
    
    system(paste0("mv output.nc run/", escenario,"/",model, "/out_",tolower(model),"_",escenario,"_", tolower(lakes[c]),"_",years_range[1],"_",years_range[length(years_range)],".nc" ))
