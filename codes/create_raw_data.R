...
for (escenario in c("picontrol")){ #"historical", "ssp126", "ssp370", "ssp585"
  ...
  for (model in c("MRI-ESM2-0", "UKESM1-0-LL", "GFDL-ESM4","IPSL-CM6A-LR", "MPI-ESM1-2-HR")){ # , ,
    print(model)
    for (c in 1:73){
      # Load NetCDF file
      ...
      outfile <- paste0("calibrated/raw_data/", escenario, "/", model, "/temp_",tolower(lakes[c]), ".csv")
